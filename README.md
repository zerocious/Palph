...
| GET | `/api/me` | монеты, стрик, сессии, питомец и таймер одним ответом |
...
Состояние главного экрана приходит одним ответом намеренно: отдельные
запросы за таймером и достижениями означали бы три резолва токена и
втрое больше SQL на ровном месте. Достижения клиент перечитывает только
после засчитанной сессии, а опрашивает сервер раз в 60 секунд в простое
и раз в 30, пока идёт таймер (обратный отсчёт всё это время живёт
локально). Ощущение свежести даёт не частота, а обновление при
возвращении в окно приложения. В сумме простой стоит 60 запросов в час
вместо 720, а трафика — около 22 КБ вместо 900.
...
Два таймера сразу запустить нельзя — иначе одно и то же время
оплачивалось бы дважды, в том числе очками недельного лидерборда, где
люди соревнуются друг с другом. Проверка симметричная: приложение не
стартует, если в Telegram идёт `TimerStates:active`, а бот не стартует,
если в `desktop_timers` лежит незакончившийся таймер. Строка с истёкшим
временем (приложение закрыли, не завершив сессию) бота не блокирует.
