...
Состояние главного экрана приходит одним ответом намеренно: приложение
опрашивает сервер раз в 15 секунд, и отдельные запросы за таймером и
достижениями означали бы три резолва токена и втрое больше SQL на ровном
месте. Достижения клиент перечитывает только после засчитанной сессии.
