...
Generates 75 PNGs (3 emotions × 5 colors × 5 accessories) + 3 emotion GIFs
...
Usage:
    ...
    # → writes 75 PNGs + 3 GIFs into <repo_root>/assets/pet/

Single-image mode (`services.PET_SINGLE_IMAGE_MODE=True`) uses a separate
`assets/pet/default.png` (1024×1024, not generated by this script). Replace
that file directly from the source PNG — do not upscale (upscaling blurs detail).
"""
...
# Drawing
# ────────────────────────────────────────────────────────────────
SIZE = 256              # placeholder PNG/GIF side in px (multi-asset mode)
PET_DEFAULT_SIZE = 1024 # single-image mode: assets/pet/default.png (manual)
CENTER = SIZE // 2
BODY_RADIUS = 80
...
def main() -> int:
    ...
    parser.add_argument(
        "--with-periods",
        action="store_true",
        help="Also write the same 75 PNG + 3 GIF into assets/pet/{morning,day,evening,night}/",
    )
