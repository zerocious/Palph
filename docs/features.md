...
- **Cooldown 7 дней** (`user_tips_seen`); если пул исчерпан — показываем повторно.
...
- **Известная проблема:** пагинация «Все советы» инкрементит `total_views`,
  что ускоряет ачивку. Фикс — не дёргать gamification hook на `tips:list`.
...
❓ FAQ или написать вопрос в чат. Полный админский набор — 20 команд,
[../admin_commands.md](../admin_commands.md).

## 11. Аккаунт и приватность

`/delete_account` — двухшаговое подтверждение, затем полное удаление
данных (см. [data-model.md](data-model.md) §«Удаление данных пользователя»).
Политика — [../PRIVACY.ru.md](../PRIVACY.ru.md).
