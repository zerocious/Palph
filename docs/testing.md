...
> **Doc sync:** 2026-09-13 · **807** тест в 52 файлах, все зелёные
...
Этого мало: проверка может записывать результаты и всё равно быть
непровальной. Поэтому есть `scripts/mutate_docs_check.py` — он вносит по
одной поломке (в документ или в код) и требует, чтобы `check_docs.py`
упал:

```bash
python scripts/mutate_docs_check.py                    # все мутации
python scripts/mutate_docs_check.py --group constants  # только числа баланса
# группы: structure | commands | audits | constants
```

За время работы над документацией этот приём нашёл **13 непровальных
проверок** — все они выглядели зелёными и ничего не проверяли. Типичные
причины: узкий класс символов в регулярке (`[a-z_]+`), из-за которого
переименование делало сущность невидимой; сравнение документа с числом,
записанным прямо в проверке, а не прочитанным из кода; сопоставление
маркера голой подстрокой (`class Breaker` находится внутри
`class BreakerRenamed`); и `skip` по условию, которое в рабочем окружении
выполняется всегда — так проверка дат `Doc sync` молча пропускалась в
shallow-клоне.

Мораль, которая стоит отдельной строки: **проверку надо проваливать
руками сразу после того, как она написана**. Зелёный отчёт не отличает
работающую проверку от сломанной.
...
| Документация | `test_docs_consistency.py` | 14 |
