...
> **Doc sync:** 2026-09-13 · 15 python-скриптов + 1 shell-шаблон.
...
|--------|--------|------------|
| `check_docs.py` | `python scripts/check_docs.py [--quiet]` | **Сверяет документацию с кодом**. Те же проверки (кроме счётчиков тестов) прогоняются внутри pytest через `tests/test_docs_consistency.py`. Список: количество таблиц и индексов, число репозиториев, таксономия событий, алиасы `/export`, ключи локалей, счётчики контента, значения фичефлагов, слэш-команды против `admin_commands.md` в обе стороны, **числа баланса** (награды за задачи, дневные капы, очки, множители, лимиты спринт-плана — всё читается из кода, не из копии внутри проверки), статус находок в аудитах (`✅ решено` требует, чтобы признак решения в коде был, `🔴 открыто` — чтобы его не было), список и порядок фич `/feature_usage`, даты `Doc sync` (не старше последнего коммита в документ, не из будущего, без пина на коммит в шапке), счётчик тестов и таблица по областям в `docs/testing.md`, все markdown-ссылки и гигиена разметки (незакрытые блоки кода, разорванные переносом код-спаны, забытые `TODO:`). Нужен только stdlib; проверка счётчиков тестов требует pytest и без него помечается `skip`. Выход `1` при расхождении — годится для CI |
| `mutate_docs_check.py` | `python scripts/mutate_docs_check.py [--group structure\|commands\|audits\|constants]` | **Мутационная проверка самого `check_docs.py`**: вносит по одной поломке в документ или в код и требует, чтобы проверка упала. Проверку, которую нельзя провалить, невозможно отличить от работающей по зелёному отчёту — этим скриптом за сессию найдено 13 таких. Мутации с `{DATE}` подставляют дату из шапки самого файла, чтобы якоря не рассыпались при обновлении `Doc sync`. Не запускать два экземпляра параллельно: восстановление одного затрёт мутацию другого |
