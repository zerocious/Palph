# User flows — Palph

Документ описывает стандартные пути пользователя в Telegram-боте Palph (v0.8) и считает клики до «полезного действия» — первого вопроса, запуска таймера, просмотра совета.

**Источник правды:** `bot.py` (клавиатуры, FSM, handlers).  
**Версия бота:** v0.8. **Тесты:** 533 (`pytest`).  
**Дата:** 2026-05-22. **Статус:** living doc — обновлять при смене навигации.

---

## Как считаем клики

| Тип | Считается |
|-----|-----------|
| Reply-кнопка (нижняя клавиатура) | 1 клик |
| Inline-кнопка (под сообщением) | 1 клик |
| Текстовый ответ (определение, @username, ЧЧ:ММ) | отдельно: «+1 ввод», не UI-клик |
| Команда `/start`, `/stop`, `/leaderboard` | 0 кликов меню (но отдельное действие) |

«Полезное действие» — момент, когда пользователь реально учится или получает контент (таймер тикает, отвечает на вопрос, читает совет).

---

## 1. Карта приложения

```mermaid
flowchart TB
    START["/start"] --> ONB{Новый пользователь?}
    ONB -->|да| SETUP["Onboarding: настроить / начать сразу"]
    ONB -->|нет| MAIN["Главное меню"]
    SETUP --> MAIN

    MAIN --> STUDY["📚 Учеба"]
    MAIN --> PROFILE["📊 Мой профиль"]
    MAIN --> FAQ["❓ FAQ"]
    MAIN --> NEWS["📢 Новости"]

    STUDY --> SUBJECTS["📖 Предметы"]
    STUDY --> TIPS["🎓 Советы для продуктивности"]
    STUDY --> TIMER_STD["⏱️ Стандартный таймер (25 мин)"]
    STUDY --> TIMER_CUSTOM["⏱️ Кастомный таймер"]
    STUDY --> BACK["🏠 Назад в меню"]

    PROFILE --> ACH["🏆 Достижения"]
    PROFILE --> SET["⚙️ Настройки"]
    PROFILE --> PROG["📊 Прогресс"]
    PROFILE --> PET["🐾 Питомец"]
    PROFILE --> FR["👥 Друзья"]
    PROFILE --> FRZ["❄️ Заморозить стрик"]

    subgraph commands["Только команды (нет кнопки в меню)"]
        LB["/leaderboard"]
        FR_CMD["/friends"]
        SHARE["/share_friend"]
    end
```

### Главное меню (Reply)

| Кнопка | Действие |
|--------|----------|
| 📚 Учеба | Подменю учёбы |
| ❓ FAQ | Inline-меню вопросов |
| 📊 Мой профиль | Текст + inline-действия |
| 📢 Новости | Статичный текст |

### Подменю «📚 Учеба» (Reply, 3 ряда)

| Ряд | Кнопка | Действие |
|-----|--------|----------|
| 1 | 📖 Предметы | FSM: предмет → режим → сессия |
| 1 | 🎓 Советы для продуктивности | Категории советов |
| 2 | ⏱️ Стандартный таймер (25 мин) | Запуск Pomodoro |
| 2 | ⏱️ Кастомный таймер | FSM: ввод минут 5–120 |
| 3 | 🏠 Назад в меню | Главное меню |

---

## 2. Onboarding (первый /start)

```mermaid
flowchart TD
    START["/start"] --> CHOICE{"🔧 Настроить сейчас<br/>🚀 Начать сразу"}
    CHOICE -->|сразу| MAIN["Главное меню<br/><b>1 клик</b>"]
    CHOICE -->|настроить| MORN["Утро: ЧЧ:ММ или /skip"]
    MORN --> EVE["Вечер: ЧЧ:ММ или /skip"]
    EVE --> MAIN2["Главное меню"]
```

| Путь | Клики | Ввод текста |
|------|-------|-------------|
| Начать сразу | 1 | — |
| Полная настройка | 1 + 0–2 шага | 0–2 раза ЧЧ:ММ или /skip |

Deep-link `?start=friend_<token>` обрабатывается после welcome — side effect, FSM onboarding не ломает.

---

## 3. Pomodoro (таймер)

```mermaid
flowchart LR
    M["Главное меню"] -->|1| S["📚 Учеба"]
    S -->|2| T25["⏱️ Стандартный 25 мин"]
    T25 --> RUN["Таймер active"]
    RUN -->|опц.| STOP["⏹️ Остановить"]
    RUN -->|опц.| BACK["⬅️ Назад в меню<br/>таймер в фоне"]
    STOP --> RATE["Inline: оценка 1–5<br/>или пропуск"]
```

| Путь | Клики до старта | После сессии |
|------|-----------------|--------------|
| Стандартный 25 мин | **2** | +1–2 inline (рейтинг), опционально push достижений |
| Кастомный | **3** | +1 текст (минуты) |
| /stop из любого экрана | 0 меню | то же, что ⏹️ Остановить |

**Заметки:** `/stop` во время MCQ/task/квиза **не** чистит чужой FSM (fix v0.7). «Назад в меню» во время таймера не останавливает сессию — только `/stop` или автозавершение.

---

## 4. Квизы — общая цепочка

Текущий порядок навигации (v0.8): **❓ Квизы → предмет → режим → (раздел / сессия)**.

```mermaid
flowchart TD
    M["Главное меню"] -->|1| S["📚 Учеба"]
    S -->|2| Q["📖 Предметы"]
    Q -->|3| SUB["Выбор предмета"]
    SUB -->|4| MODE["Выбор режима"]
    MODE --> BRANCH{Режим}

    BRANCH -->|situational| SEC["Раздел I–IV"]
    SEC -->|5| ANS["Текстовый ответ"]

    BRANCH -->|mcq| MCQ["Inline: вариант"]
    BRANCH -->|flashcards| FC1["Inline: Показать ответ"]
    FC1 --> FC2["Inline: оценка"]
    BRANCH -->|tasks| TASK["Текстовый ответ"]
```

### Режимы учёбы (`STUDY_MODES`)

| id | Кнопка | Контент |
|----|--------|---------|
| situational | 🎯 Ситуационные квизы | `situational/section-*.txt` |
| flashcards | 🃏 Флэш-карты | `flashcards.txt` + свои карточки |
| mcq | ❓ Тест с выбором ответа | `mcq.txt` |
| tasks | 📷 Задачи (фото или текст) | `tasks/task-*.json` [+ PNG]; опционально `groups.json` |

Предметы (`SUBJECTS`): 🏭 ОПМ, 🧮 Математика, 🇬🇧 Английский. В меню показываются только предметы с непустым контентом (`available_subjects`).

### Клики до первого задания

| Режим | Reply | Inline | Итого до 1-го вопроса |
|-------|-------|--------|----------------------|
| MCQ | 4 | 1 | **5** |
| Флэш-карты | 4 | 2 | **6** |
| Ситуационный | 5 | 0 | **5** + текст ответа |
| Задачи (фото/текст) | 4 | 0–1 (группы) | **4–5** + текст ответа |

### Extra UI после выбора предмета

После шага «предмет» бот отправляет **второе** сообщение с inline:

- ➕ Добавить карточку (`fc_add`)
- 📇 Мои карточки (`fc_list`)

Не обязательно для учёбы, но увеличивает шум на экране выбора режима.

---

## 5. Ситуационный квиз

```mermaid
sequenceDiagram
    participant U as Пользователь
    participant B as Бот

    U->>B: 📚 Учеба
    U->>B: ❓ Квизы
    U->>B: Предмет (напр. 🏭 ОПМ)
    U->>B: 🎯 Ситуационные
    U->>B: Раздел I
    B->>U: «Напиши определение термина…»
    U->>B: текст ответа
    B->>U: фидбек → следующий термин
```

| Этап | Клики |
|------|-------|
| До первого термина | **5** reply |
| Следующий термин в том же разделе | **0** меню, только текст |
| Выход | 🛑 Завершить квиз → 📚 Учеба (1) |

Разделы I–IV показываются только если файл `section-*.txt` непустой (`available_quiz_sections`).

---

## 6. MCQ

| Этап | Клики |
|------|-------|
| До 1-го вопроса | **5** (4 reply + 1 inline) |
| Каждый ответ | 1 inline |
| Завершение сессии | 🛑 Завершить (1) → подменю учёбы |

Вопросы — 4 inline-варианта на сообщение; сессия идёт до конца списка или 🛑.

---

## 7. Флэш-карты (SM-2)

```mermaid
flowchart LR
    CARD["Карточка: термин"] --> SHOW["💡 Показать ответ"]
    SHOW --> RATE["❌ Не знал / 😐 Сложно / ✅ Легко"]
    RATE --> NEXT["Следующая по SM-2"]
```

| Этап | Клики |
|------|-------|
| До 1-й карточки | **6** (4 reply + 2 inline) |
| Каждая карточка | **2** inline минимум |
| Источник карточек | Настройка ⚙️: mix / official / own |

Если карточек нет — сообщение с подсказкой + возврат к режимам предмета.

---

## 8. Задачи с картинкой

| Этап | Клики |
|------|-------|
| До 1-й задачи | **4** reply |
| Ответ | текст (до 3 попыток, затем solution image) |
| Награда | +3 / +2 / +1 🪙 по попытке |

---

## 9. Советы по продуктивности

```mermaid
flowchart TD
    M["Главное меню"] -->|1| S["📚 Учеба"]
    S -->|2| T["🎓 Советы"]
    T -->|3| CAT["Категория"]
    CAT --> TIP["Совет + inline"]
    TIP --> MORE["🔄 Ещё совет"]
    TIP --> LIST["📋 Все советы"]
    LIST --> PAGE["◀️ / ▶️ пагинация"]
```

| Категория (Reply) | id |
|-------------------|-----|
| ⏰ Тайм-менеджмент | tm |
| 🧠 Техники запоминания | mem |
| 🎯 Как пользоваться ботом | bot |
| 🔗 Ссылки на статьи и книги | links (URL-кнопки) |

| Путь | Клики до контента |
|------|-------------------|
| Случайный совет | **3** reply |
| Ещё совет | +1 inline |
| Список / листание | +2+ inline |

**Геймификация:** +1🪙/день за первый просмотр; ачивка «Любознательный» (10 советов). Known issue: пагинация «Все советы» инкрементит `total_views` — см. [TODO.md](TODO.md) #18.

**Параллельный канал:** «совет дня» в утреннем push (`build_morning_tip_block`) — 0 кликов.

---

## 10. Профиль и настройки

```mermaid
flowchart TD
    M["Главное меню"] -->|1| P["📊 Профиль"]
    P -->|2 inline| SET["⚙️ Настройки"]
    SET --> TOG["Вкл/Выкл: утро, вечер, стрик, ачивки"]
    SET --> TIME["🕘 Изменить время"]
    TIME --> TEXT["Ввод ЧЧ:ММ"]
    SET --> TZ["🌍 Часовой пояс → список"]
    SET --> PRIV["👤 Видимость в LB"]
    SET --> FC_SRC["🃏 Источник флэш-карт"]
    SET --> FC["📇 Мои карточки"]
    FC --> SUBJ["Предмет"] --> LIST["Список / 🗑"]
```

| Задача | Клики (типично) |
|--------|-----------------|
| Открыть настройки | **2** |
| Toggle уведомления | **3** |
| Сменить время утра/вечера | **4+** (+ текст) |
| Мои карточки → добавить | **4+** через настройки |

### Два входа в «Мои карточки»

1. **Профиль → ⚙️ Настройки → 📇 Мои карточки** (4+ клика)
2. **Квизы → предмет → inline ➕ / 📇** (3 клика до предмета + 1 inline)

Рекомендация при оптимизации: оставить один канонический путь.

---

## 11. Питомец, друзья, стрик

```mermaid
flowchart LR
    P["Профиль"] -->|2| PET["🐾 Питомец"]
    PET -->|3| COL["🎨 Цвета / 🎁 Аксессуары"]
    COL -->|4| ACT["Купить 💰 / Надеть ✓"]

    P -->|2| FR["👥 Друзья"]
    FR -->|3| ADD["➕ Добавить друга"]
    ADD --> TXT["@username или ID"]

    P -->|2| FRZ["❄️ Заморозить стрик"]
    FRZ -->|3| BUY["✅ Купить"]
```

| Задача | Клики |
|--------|-------|
| Экран питомца | 2 |
| Покупка аксессуара | 4–5 (+ confirm) |
| Список друзей | 2 |
| Добавить друга | 3 + текст |
| Invite-link | `/share_friend` → share → друг жмёт link (вне меню) |
| Заморозка стрика | 3–4 |

**Лидерборд:** только `/leaderboard` — кнопки в главном/профильном меню нет.

---

## 12. FAQ

10 тем (`faq.mission` … `faq.why_free`), RU/EN через `locales/*.json`.

| Шаг | Клики |
|-----|-------|
| ❓ FAQ | 1 reply |
| Тема вопроса | 1 inline |
| Назад к списку | 1 inline «◀️ К списку» |

---

## 13. Сводка: узкие места

Главный overhead — **ветка квизов**, не таймер и не профиль.

```
Главное → Учеба → Квизы → Предмет → Режим → [Раздел] → действие
   1        2       3         4          5        6?
```

Для returning user повторяется цепочка 4–5 reply-кликов каждый раз.

---

## 14. Идеи оптимизации (приоритет)

| # | Проблема | Сейчас | Идея | Экономия |
|---|----------|--------|------|----------|
| 1 | Лишний уровень «Учеба» для квизов | 2 клика до предмета | «❓ Квизы» в главном меню | −1 |
| 2 | Нет «продолжить» | 4–5 каждый раз | `last_subject` + `last_mode` → «▶️ Продолжить MCQ · ОПМ» | −2…−3 |
| 3 | Ситуационный + раздел | 5 | «Продолжить раздел I» / SRS-default | −1 |
| 4 | Два входа в свои карточки | 3–4 vs 4+ | Один канонический путь | ясность UX |
| 5 | Inline после предмета | +2 кнопки всегда | Показывать только при `own` cards / по запросу | меньше шума |
| 6 | Лидерборд | только команда | Кнопка в профиле | discoverability |
| 7 | Флэш: 2 tap/карта | 6 до старта | «Показать сразу» или одна кнопка оценки | −1/карта |
| 8 | Sprint-план (бэклог) | — | «📋 Сегодня» = 1–2 клика до плана дня | см. [BACKLOG.md](BACKLOG.md) |
| 9 | Задача в спринте | — | — | [TODO.md](TODO.md) #21 |

### Целевые метрики

| Flow | Сейчас | Цель после оптимизации |
|------|--------|------------------------|
| Pomodoro 25 мин | 2 | 2 (без изменений) |
| MCQ (returning) | 5 | 2–3 |
| Ситуационный ОПМ | 5 | 3–4 |
| Случайный совет | 3 | 1–2 |
| Настройки | 2 | 2 |

---

## 15. Связанные документы

| Файл | Связь |
|------|-------|
| [README.md](README.md) | Обзор продукта, быстрый старт |
| [TODO.md](TODO.md) | Задачи по UX (tips pagination #18) |
| [BACKLOG.md](BACKLOG.md) | Sprint / adaptive plan — сокращение «что учить» |
| [LEADERBOARD.md](LEADERBOARD.md) | Друзья, freeze, privacy |
| [tips/README.md](tips/README.md) | Поведение советов |

---

*При изменении клавиатур или FSM в `bot.py` обновляй этот файл.*
