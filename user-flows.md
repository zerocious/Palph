...
**Версия бота:** v0.8. **Тесты:** 807 (`pytest --collect-only -q`).  
**Doc sync:** 2026-09-13. **Статус:** living doc — обновлять при смене навигации.  
...
```mermaid
flowchart TD
    START["/start"] --> LANG["Inline-пикер языка<br/>ru / en"]
    LANG --> CHOICE{"🔧 Настроить сейчас<br/>🚀 Начать сразу"}
    CHOICE -->|сразу| MAIN["Главное меню"]
    CHOICE -->|настроить| MORN["Утро: ЧЧ:ММ или /skip"]
    MORN --> EVE["Вечер: ЧЧ:ММ или /skip"]
    EVE --> MAIN2["Главное меню"]
```

Пикер языка показывается новому пользователю **и** любому, у кого
`users.locale` пуст; выбор сразу переустанавливает команды в `/`-пикере.

| Путь | Клики | Ввод текста |
|------|-------|-------------|
| Начать сразу | 1 (язык) + 1 | — |
| Полная настройка | 1 (язык) + 1 + 0–2 шага | 0–2 раза ЧЧ:ММ или /skip |
...
flowchart LR
    ...
    STOP --> RATE["Inline: оценка 😞😐🙂😍<br/>(1–4) или пропуск"]
```

| Путь | Клики до старта | После сессии |
|------|-----------------|--------------|
| Стандартный 25 мин | **2** | +1 inline (оценка 1–4 или «пропустить»), опционально push достижений |
| Кастомный | **3** | +1 текст (минуты) |
| /stop из любого экрана | 0 меню | то же, что ⏹️ Остановить |

**Заметки:** `/stop` во время MCQ/task/квиза **не** чистит чужой FSM (fix v0.7). «Назад в меню» и **📖 Подготовка** во время таймера не останавливают сессию — таймер идёт в фоне; остановить: `/stop` или дождаться автозавершения.
...
Предметы в «📖 Подготовка»: 🧮 Математика, 📊 Бухучёт (если есть контент), 🇬🇧 Английский. **ОПМ временно скрыт** (`PREP_HIDDEN_SUBJECT_IDS`). В меню показываются только предметы с непустым контентом (`available_subjects`).
...
sequenceDiagram
    ...
    U->>B: 📖 Подготовка
    U->>B: Предмет с ситуационным контентом
    U->>B: 🎯 Ситуационные
    U->>B: Раздел I
...
> **Сейчас этот флоу недостижим:** единственный предмет с
> `situational/` — ОПМ, а он скрыт через `PREP_HIDDEN_SUBJECT_IDS`.
> То же с MCQ (§6). Флоу описан на будущее и заработает, как только у
> видимого предмета появится соответствующий файл.
...
Как и ситуационные квизы (§5), режим сейчас недостижим: `mcq.txt` непустой
только у скрытого ОПМ.
...
## 8. Задачи (фото или text-only)

| Этап | Клики |
|------|-------|
| До 1-й задачи | **3–4** reply (опционально выбор группы `groups.json`) |
| Ответ | текст (до 2 попыток) |
| 1-я ошибка | подсказка (`hint`) или «попробуй ещё» |
| 2-я ошибка | ответ / полное решение |
| Награда | +3 / +2 / 0 🪙 по попытке |
...
**Геймификация:** +1🪙/день за первый просмотр; ачивка «Любознательный» (10 советов). **Листание «📋 Все советы» просмотром не считается** (только cooldown) — иначе ачивку можно было нафармить перелистыванием; «🔄 Ещё совет» считается.
...
flowchart TD
    ...
    UT --> SUBJ2["Предмет"] --> IMP["Импорт .txt / 🗑"]
```

Полный порядок кнопок настроек — [docs/features.md](docs/features.md) §9.
...
2. **📖 Подготовка → предмет → inline ➕ / 📇** (3 клика до предмета + 1 inline)
