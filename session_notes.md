...
**Итог:** 5 из 6 пунктов плана закрыты + 9 бэклог-пунктов (admins→БД,
...
foundation для funnel/cohort/path analysis) + bonus-fix
`stop_active_timer` + GitHub push (`zerocious/Palph`). Pytest:
**136 тестов** (~12 сек). Остался только пункт #16 (полноценный питомец) —
