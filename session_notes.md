...
**Итог:** 5 из 6 пунктов плана закрыты + 7 бэклог-пунктов (admins→БД,
...
гарантия эффективности) + bonus-fix `stop_active_timer` + GitHub push
(`zerocious/Palph`). Pytest: **106 тестов** (~10 сек). Остался только
пункт #16 (полноценный питомец) — самая большая фича, разбита на
код-трек и арт-трек.
