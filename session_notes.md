...
## Session — 2026-09-13 (сверка №9: аудиты врали о себе, дата сверки — тоже)

Goal: продолжить сверку и улучшение документации.

**Итог:** десять находок в аудитах, описанных как открытые проблемы, давно
закрыты в коде; правило «обнови `Doc sync`» нарушено в 36 документах —
самим автором правила; пин на коммит в шапках протух. Три новые проверки,
все три провалились на мутациях, как положено.

### Аудиты описывали решённые проблемы как открытые

`audits/error-handling-review.md` (22 мая) и
`audits/system-resilience-review.md` (24 мая) читаются как список того,
чего в проекте нет. Сверил каждую находку с кодом: из **20** находок закрыты **10** —
централизованный `@router.errors()`, `TelegramRetryAfter`, retry с
back-off, circuit breaker, `add_done_callback`, graceful shutdown,
таймаут Telegram-сессии, `PRAGMA busy_timeout`, retry на
`database is locked`, семафор на исходящие. Открытыми остались **8** (своей
иерархии исключений нет, `not modified` ловится общим `except`, хелпера
авторизации нет, утечка текста исключения в админский UI и прочее), ещё
две — частично, точечного признака у них нет.

Текст находок не переписывал — это датированная запись, её ценность в том,
что она фиксирует состояние на дату. Вместо этого в начале каждого аудита
— таблица «Статус находок» с колонкой **признак в коде**, и
`check_docs.py` держит её честной: «✅ решено» требует, чтобы признак в
коде был, «🔴 открыто» — чтобы его не было.

Дважды выбрал признак неправильно, и оба раза по одной причине: брал
**симптом проблемы**, а не **символ решения**. Для находки 5.1 (утечка
исключения в UI) сначала поставил сам протекающий формат строки — тогда у
половины строк таблицы логика проверки оказывалась перевёрнутой. Признак
— это всегда то, что появится в коде, когда находку закроют. Соглашение
записано прямо в таблице.

### Маркер как голая подстрока — та же ловушка, что с регулярками

Первая версия проверки искала признак простым `in`, и маркер
`class TelegramSendBreaker` находился внутри
`class TelegramSendBreakerRenamed` — то есть переименование решения
осталось бы незамеченным. Это четвёртый
случай одной и той же ошибки за сессию (узкий класс символов →
переименование невидимо). Сопоставление стало граничным (`(?<!\w)` /
`(?!\w)`), мутация «решение убрано из кода» ловится.

### Правило `Doc sync` не соблюдал автор правила

В `docs/development-guide.md` с прохода №5 висит пункт чеклиста: «в шапке
правленого документа обновлён `Doc sync`». Проверил механически — дата
старше последней правки в **36 документах из 36 датированных**. Проходы
5–9 правили документы и оставляли `2026-09-05`. Дата сверки без гарантии
хуже отсутствия даты: читатель ей верит.

Теперь `check_doc_sync_dates` сравнивает дату в шапке с датой последнего
коммита в этот файл (один `git log` на всё дерево) и заодно ловит дату из
будущего. Все 36 дат подняты на сегодня — и этот коммит их же и трогает,
так что правило выполняется ровно тем, что оно требует.

### Тринадцатая непровальная проверка — моя собственная, свежая

Первая версия `check_doc_sync_dates` начиналась с «если clone shallow —
пропустить»: в контейнере сессии clone именно shallow, и проверка
**скипалась всегда**. Ровно то, против чего вся эта машинерия и писалась,
— только что написанное своими руками.

Починка точнее, чем отказ: граничный коммит shallow-клона выглядит как
корневой и «добавляет» всё дерево целиком, поэтому пропускаются не все
файлы, а только безродительские коммиты. Файлы, правленные внутри
доступной истории (то есть все, которые кто-то трогает), проверяются.

### Проверка поймала промах в собственном коммите

Первая версия сравнивала дату в шапке только с датой последнего коммита,
то есть узнавала о забытом `Doc sync` **после** коммита. Ровно это и
случилось: `docs/scripts.md` я правил в этом же проходе, дату не поднял, и
проверка зажглась сразу после `git commit`. Теперь незакоммиченная правка
считается правкой сегодняшним днём — промах виден до коммита, а не после.

### Пин на коммит в шапке не может быть правдой

Семь документов заявляли «код на коммите `0ac30af`». Код с тех пор
сдвинулся — фикс листания советов (`5c48adb`) правил `bot.py`. Главное:
пин в живом документе **принципиально** не поддерживаем — документ не
может назвать коммит, который его же и содержит, поэтому пин всегда
отстаёт на один код-коммит. Пины убраны, свежесть заявляет `Doc sync`;
проверка не даёт им вернуться в шапку. В аудитах и журнале сессий пин
остаётся — это датированные записи.

### `/feature_usage`: сверяются имена, а не число

Проход №4 нашёл вручную, что пример в справочнике показывал 8 строк при
14 реальных фичах. Теперь имена фич разбираются из
`compute_feature_usage` через AST и сверяются с `admin_commands.md`
целиком и **по порядку** — переименование фичи в коде не пройдёт мимо.
Счётчик «N фич» в документах сверяется с тем же списком (порог «≥3 фичи с
данными» из приёмочных критериев при этом не путается со счётчиком).

### Харнесс: якоря с датами больше не рассыпаются

Мутации «откатить `Doc sync` назад» и «поставить дату в будущее»
неизбежно привязаны к дате в файле — а она меняется каждый проход.
Вместо этого в мутации пишется `{DATE}`, и харнесс подставляет дату из
шапки самого файла.

### Verification

- `python -m pytest -q` → **807 passed** (+2: две новые проверки внутри
  `test_docs_consistency.py`)
- `python scripts/check_docs.py` → **70/70**
- `python scripts/mutate_docs_check.py` → **поймано 36/36**, дерево чистое
- `python -m py_compile bot.py services.py repository.py db.py`, `import bot`,
  `scripts/audit_i18n_keys.py` — чисто

### Deferred

- Проверки так и не заведены в CI — в репозитории нет workflow, и заводить
  его — решение владельца, а не правка документации.
