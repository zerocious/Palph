...
## Session — 2026-09-07 (сверка №8: команды под охраной, харнесс в репозиторий)

Goal: продолжить сверку и улучшение документации.

**Итог:** тот же дефект регулярки найден **в третий раз**; добавлена
проверка команд; мутационный харнесс переехал из `/tmp` в репозиторий.

### Сначала — подтверждение вчерашнего фикса

Дата перевалила на 7 сентября (тот самый переход, что вчера всё сломал).
Оба клока снова совпали (`2026-W37`), 803 теста зелёные. Фикс держит.

### Узкий класс символов — третий раз подряд

Мутация «переименовать раздел `/backup` в `/backupX`» не была поймана:
регулярка разделов для заголовков вида `### /команда` матчила только
строчную часть, то
есть `/backupX` разбиралось как `backup` и выглядело задокументированным.

Это ровно та же ошибка, что и с событиями (пропуск №6) и алиасами: **узкий
класс делает переименование невидимым**. Прошёлся по всем классам символов
в `check_docs.py` и заменил на `[A-Za-z0-9_]+` там, где разбираются
идентификаторы из кода и документов. После этого 4/4.

### Новая проверка: слэш-команды

Код против справочников, в обе стороны: у каждой админ-команды есть раздел
в `admin_commands.md`; в справочнике нет несуществующих команд; список в
README совпадает с кодом; все команды `/`-пикера имеют хендлер и
перечислены в README. Именно так вручную (проход №4) нашлось, что `/help`
— единственная админ-команда без раздела.

### Харнесс — в репозиторий

`scripts/mutate_docs_check.py`: 27 мутаций в трёх группах (структура,
команды, числа баланса). Всю сессию он жил в `/tmp` и переписывался заново
— теперь это инструмент проекта с описанием в `docs/scripts.md` и
`docs/testing.md`.

Заодно поймал собственную ошибку эксплуатации: два харнесса, запущенные
параллельно, затёрли восстановление друг друга и оставили в `bot.py`
мутацию `QUIZ_INTERVALS = [1, 3, 5, 9]`. Обнаружил это не я, а
`check_docs.py` — сказал, что код разошёлся с документацией. В докстринге
скрипта теперь явное предупреждение.

### Что ещё сверено (совпало)

- `/content_stats`: все 7 sub-view и 4 бакета EF — как в `admin_commands.md`.
- `render_friends_tab`: сортировка по `total_final`, медали 🥇🥈🥉,
  маркер `(Вы)`, множитель применяется — как в документации.
- `StreakService`: miss-day path сначала пробует `consume_freeze_if_active`,
  только потом `apply_streak_reset`; есть уведомление `reminders.freeze_used`.

### Verification

- `python -m pytest -q` → **804 passed**
- `python scripts/check_docs.py` → 60/60
- `python scripts/mutate_docs_check.py` → **поймано 27/27**, дерево чистое

---

## Session — 2026-09-06 (сверка №7: неполный фикс догнал через сутки)

Goal: продолжить дотошную сверку и улучшить документацию.

**Итог:** прогон покраснел на пяти тестах — и виноват был **мой же
неполный фикс из первого прохода**. Плюс новая проверка чисел баланса,
которая с первой попытки оказалась наполовину пустышкой.

### Главное: правило про время я применил только наполовину

Дата перевалила за 6 сентября, и пять тестов
`test_leaderboard_service.py` упали. Продакшн-код я не трогал.

Причина: в первом проходе я сделал якорь относительным в
`test_integration_flows.py` — **привязав его к `Europe/Moscow`**, как
`users.timezone` по умолчанию. А в `test_leaderboard_service.py` написал
наивный `datetime.now()`, то есть системные часы контейнера (UTC).
Совпадало до вечера воскресенья: в 21:00 UTC в Москве уже понедельник —
**следующая ISO-неделя**. Тест писал очки в W36, `render_leaderboard`
читал W37.

```
naive (UTC контейнера): ('2026-09-06', '2026-W36')
Europe/Moscow         : ('2026-09-07', '2026-W37')
```

Правило в `docs/testing.md` §1 усилено: якорь должен быть не просто
относительным, а **в том же часовом поясе, что использует код под
тестом**. Туда же добавлен прогон под разными `TZ` как способ это
проверять — суита зелёная под UTC, Москвой, Лос-Анджелесом, Токио,
Оклендом и Киритимати (UTC−8 … UTC+14).

### Новая проверка чисел баланса — и снова пустышка с первой попытки

Добавил в `check_docs.py` сверку наград, капов, очков, множителей и
лимитов спринт-плана. Мутировал 14 констант в коде: **7 не пойманы**.

Причина ровно та же, что вчера, только в новом обличье: для этих семи я
сравнивал документ с **числом, записанным прямо в проверке**, а не
прочитанным из кода. Ребаланс такую проверку переживает целиком.
Переписал так, что каждое значение извлекается из исходника
(`TASK_REWARDS_BY_ATTEMPT`, `task_count < N`, `task_pts + N`,
`TOP_N_DISPLAY`, …). Повтор: **14 из 14 пойманы**.

Ещё один шаблон был слишком широким: мутация `TOP_N_DISPLAY` 20→10
проходила, потому что «топ-10» находилось в тексте про бонус топ-10 %.
Уточнён до `Отображается топ-{v} (\`TOP_N_DISPLAY\`)`.

### Что ещё сверено (совпало)

- `plan_service`: `SPRINT_DAYS=14`, `MIN_PLAN_ITEMS=10`,
  60/120/180/240 → 8/16/24/32, `MODE_ROTATION`, `BADGE_KEYS` — всё как в
  `docs/features.md` §12; `items_per_day` для неизвестного бюджета
  откатывается к 8.
- `/activation`: медиана и p75 по шести событиям — список в
  `admin_commands.md` совпал точно; в `docs/analytics.md` уточнил
  «перцентили» → «медиана и p75».
- Наивные `datetime.now()` в остальных тестах проверены: у бэкапов и
  таймера тот же наивный клок, что в коде, — консистентно.

### Verification

- `python -m pytest -q` → **803 passed** (802 + новая проверка констант)
- под пятью разными `TZ` → везде 803 passed
- `python scripts/check_docs.py` → 54/54
- мутации: 13/13 структурных + 14/14 констант

---

## Session — 2026-09-05 (сверка №6: проверка самой проверки)

Goal: перепроверить себя максимально дотошно и ещё улучшить документацию.

**Итог:** оказалось, что вчерашний `check_docs.py` слабее, чем выглядел —
**5 проверок из 8 нельзя было сломать переименованием**. Исправлено,
проверено мутациями, проверки заведены в pytest.

### Главная находка: проверки, которые нельзя провалить

Прогнал по каждой проверке свою мутацию — «проверка, которую нельзя
сломать, бесполезна». Из восьми поймали только три.

| Мутация | Было | Причина |
|---------|------|---------|
| Переименовать событие в коде | **не поймана** | регулярка `[a-z_]+` не матчила имя с цифрой, и событие просто **исчезало из выборки** вместо того, чтобы стать «недокументированным» |
| Переименовать алиас `/export` | **не поймана** | та же регулярка, та же причина |
| Переименовать вложенный ключ локали | **не поймана** | сравнивались только имена групп верхнего уровня и общее количество — оба не меняются |
| Переименовать таблицу | **не поймана** | сверялось только количество таблиц |
| Удалить задачу по математике | **не поймана** | подстрока `"71" in text` находилась внутри других чисел |

Общий корень у первых двух: **регулярка молча сужала выборку**, то есть
изменение превращалось в невидимость. Худший вид ложного зелёного.

### Что исправлено в `check_docs.py`

- `[a-z0-9_]+` вместо `[a-z_]+` для событий и алиасов + защита «событий
  вообще найдено ≥ 25» на случай, если регулярка снова перестанет матчить;
- обратные проверки: событие/алиас **удалили из кода, а из документа
  забыли** — теперь тоже провал;
- имена таблиц сверяются поимённо, а обзорная таблица в `data-model.md`
  разбирается построчно (первый вариант ловил колонки и дал ложное
  срабатывание — правил проверку, а не документ);
- полные пути ключей локалей (`nav.main_menu`), а не только группы;
- `\b`-границы вместо подстрок в счётчиках контента;
- число тест-файлов тоже под охраной — я обнаружил «51 файл» вручную,
  когда их стало 52;
- новая проверка гигиены разметки.

**Повторный прогон мутаций: 13 из 13 пойманы.**

### Документация как часть прогона

`tests/test_docs_consistency.py` — те же проверки внутри pytest, чтобы
расхождение краснело обычным `pytest -q`. Плюс тест
`test_every_check_actually_asserts_something`: проверка, не записавшая ни
одного результата, — пустышка.

### Вычитка

Написал точный вычитыватель (первый вариант считал ограждение блока кода
непарным бэктиком, а `func()` — битой ссылкой; чинил инструмент, не документы).
Нашёл 7 код-спанов, разорванных переносом строки, в
`audits/error-handling-review.md` — склеены.

Ещё одна попытка была отброшена как переусложнение: проверка, сверяющая
заявленное в документации **число проверок**, самореферентна — добавление
проверки меняет число, которое она проверяет. Вместо этого число из прозы
убрано: лучший способ починить цифру, которая гниёт, — не называть её.

### Кросс-документная сверка

Скрипт сравнил одинаковые факты в разных файлах. Два «расхождения»
оказались артефактами регулярки и проверены по коду: «306 vs 443» — это
использованные ключи против всех в бандле; «20 vs 28 таблиц» — экспорт
против схемы; «50 событий» в `admin_commands` — лимит `/event_timeline`
(`limit=50` в обоих местах вызова).

### Verification

- `python -m pytest -q` → **802 passed** (801 + 1 новая проверка разметки)
- `python scripts/check_docs.py` → 36/36
- мутации: 13/13 пойманы; `import bot`, `py_compile`, i18n — чисто

---

## Session — 2026-09-05 (сверка №5 + инструмент против дрейфа документации)

Goal: перепроверить себя ещё раз и улучшить документацию.

**Итог:** ad-hoc проверки, которыми я четыре раза подряд ловил расхождения,
превращены в `scripts/check_docs.py`; добавлены три диаграммы, глоссарий и
задачный индекс.

### Проверка собственного фикса

- Оба вызывающих места `_on_tip_viewed` пересмотрены заново — флаг
  прокинут верно.
- `_pick_tip` имеет фолбэк `pool or list(tips)`, поэтому исчерпание пула
  от листания не ломает выдачу. Поведение дособытийное, я его сохранил
  намеренно — задокументировал в `docs/features.md` §8.
- Тест `test_list_handler_passes_count_view_false` был на
  `inspect.getsource` — хрупкий белый ящик. Заменён на два поведенческих
  теста, которые прогоняют настоящие `handle_tips_list` /
  `handle_tips_more` через фейковый CallbackQuery и проверяют эффект в БД.
- **Мутационная проверка:** вернул `count_view=True` в
  `handle_tips_list` — тест падает; вернул фикс — проходит. Регрессия
  действительно ловится, а не декларируется.

### `scripts/check_docs.py` — 25 проверок

Причина: за сессию я четырежды писал одноразовые скрипты, ловившие дрейф.
Теперь это один инструмент, только stdlib (pytest нужен лишь для счётчика
тестов, без него — `skip`), выход `1` при расхождении.

Сверяет: markdown-ссылки; количество таблиц и индексов из `db.py`; число
репозиториев; таксономию событий против всех `event_repo.log`; алиасы
`/export` против `EXPORTABLE_TABLES`; паритет и количество ключей локалей;
счётчики контента; **значения фичефлагов** (флаг поменяли в коде — упадёт);
счётчик тестов и всю таблицу по областям в `docs/testing.md`.

**Мутационная проверка инструмента** — подделал счётчик в README, убрал
событие из таксономии, сломал ссылку, рассинхронил таблицу областей,
перевернул `PET_SINGLE_IMAGE_MODE` в коде: пойманы все пять.

Сам скрипт при первом прогоне дал два ложных срабатывания (частная цифра
«110 pytest в аналитических модулях» и историческое «732 на момент
закрытия фазы»), плюс я нашёл у себя в нём опечатку в списке маркеров.
Правил проверку, а не документы.

### Улучшения документации

| # | Что |
|---|-----|
| 1 | `docs/architecture.md` — диаграмма жизненного цикла таймера (единственное состояние, переживающее рестарт: FSM, detach на «Подготовку», три ветки reconcile) |
| 2 | `docs/data-model.md` — карта 28 таблиц по шести кластерам вокруг `users`; пунктир = связь без FK, то есть ручная чистка при удалении аккаунта |
| 3 | `docs/features.md` — путь очка от действия до лидерборда: почему дневные капы живут отдельно от недельной суммы и почему множитель не хранится |
| 4 | `docs/glossary.md` — термины проекта (mastery, тир, кап, rollover, сегмент, EF, reconcile, фичефлаг) + раздел про исторические имена (`studybuddy*`, ключ `kb.quizzes`) |
| 5 | `docs/README.md` — задачный индекс «хочу сделать X → читай Y» вместо только ролевого |
| 6 | Чеклист перед коммитом, `docs/testing.md`, `docs/scripts.md`, README — везде добавлен `check_docs.py` |

### Verification

- `python -m pytest -q` → **793 passed** (792 + 1: один хрупкий тест заменён двумя)
- `python scripts/check_docs.py` → 25/25
- `import bot`, `py_compile`, `audit_i18n_keys` → чисто

---

## Session — 2026-09-05 (фикс: листание советов больше не фармит ачивку)

Goal: закрыть единственный незакрытый дефект из документации — known issue
TODO #18, который висел с 2026-05-22.

**Итог:** листание «📋 Все советы» перестало считаться просмотром совета.

### Проблема

`_edit_or_send_tip` вызывал `_on_tip_viewed` безусловно, а его использовали
оба callback'а — и «🔄 Ещё совет» (`tips:more`), и пагинация
(`tips:list` ◀️/▶️). Значит каждое нажатие стрелки:

- инкрементило `user_tips_stats.total_views` → ачивку «Любознательный»
  (10 просмотров, +30 🪙) можно было получить перелистыванием одной
  категории, ничего не читая;
- писало событие `tip_viewed` → метрика «сколько советов прочитано» и
  feature-adoption по советам были завышены.

Дневная монета не затрагивалась — она и так 1/день.

### Решение

Ровно то, что было записано в BACKLOG как подход: разделить «просмотр» и
«показ». Флаг `count_view` (keyword-only, по умолчанию `True`):

| Точка входа | count_view | Что делает |
|-------------|:----------:|------------|
| Категория (`_send_random_tip`) | `True` | seen + views + монета + ачивка + событие |
| «🔄 Ещё совет» (`tips:more`) | `True` | то же |
| Листание (`tips:list`) | **`False`** | только `record_seen` для cooldown |

### Changes

| # | Area | Change |
|---|------|--------|
| 1 | `bot.py` | `_on_tip_viewed(..., *, count_view=True)` — при `False` ранний выход после `record_seen`; `_edit_or_send_tip` прокидывает флаг; `handle_tips_list` передаёт `count_view=False` |
| 2 | Tests | `tests/test_tips_gamification.py` — 5 регрессионных тестов: views не растут, событие не пишется, ачивка не двигается, seen пишется, «Ещё совет» по-прежнему считается |
| 3 | Docs | TODO #18, BACKLOG (→ shipped), tips/README, user-flows §9, docs/features §8, docs/analytics (семантика `tip_viewed`), счётчик тестов 787 → 792 |

### Verification

- `python -m pytest -q` → **792 passed** (787 + 5 новых)
- Таблица тестов по областям в `docs/testing.md` пересчитана скриптом: все
  12 строк сходятся, сумма 792
- `python -c "import bot"` → OK

### Note

Второй известный дефект из документации — недостижимость ситуационных
квизов и MCQ (находка №34) — **не трогал**: ОПМ скрыт намеренно и
временно (коммит `eef0f05`, «temporarily hide»), а ждёт он контента
(разделы II–IV, TODO #1), а не кода. Разблокировать его — продуктовое
решение владельца.
...
| 21 | PRIVACY.md / PRIVACY.ru.md §2.1 | «Имя, фамилия, language code, message ID, chat ID не сохраняются» | `language_code` пишется в событие `user_registered`; `chat_id` — в ключ `fsm_storage`; имя, фамилия и message ID — в `messages.log` при обращении в поддержку |
| 22 | PRIVACY §2.3 | «Ни одно событие не содержит свободный текст» | `pet_renamed` хранит имя питомца (20 символов) |
| 23 | PRIVACY §2.4, §7 | `messages.log` и состояние FSM отсутствовали в перечне данных и в сроках хранения | Добавлены обе категории |
| 24 | PRIVACY §8 | «`/delete_account` — все данные удаляются» | Уточнено: файловые `bot.log` и `messages.log` командой не покрываются |
| 25 | admin_commands.md `/funnel` | Пример вывода из 6 шагов | В коде 11 шагов + отдельный event-only блок |
| 26 | admin_commands.md `/feature_usage` | Пример из 8 строк при заявленных 14 фичах | Приведены все 14 в порядке вывода |
| 27 | admin_commands.md `/backup`, audits | «в Docker — `/data/backups/`» | `/app/data/backups/` (та же путаница, что чинили в коде коммитами `057770c`, `3d1daef`) |
| 28 | admin_commands.md, docs/features | «20 админских команд» | **21**; из них 4 только для главного админа |
| 29 | admin_commands.md | Не было раздела про `/help` — единственная незадокументированная админ-команда | Раздел добавлен |
| 30 | user-flows.md §3 | «Inline: оценка 1–5» | Оценка **1–4** (😞😐🙂😍), `RATING_EMOJIS` |
| 31 | user-flows.md §2 | Онбординг начинался с выбора «настроить / сразу» | Перед ним показывается inline-пикер языка |
| 32 | user-flows.md §10 | В схеме настроек не было «🌐 Язык» и «📄 Мои задачи» | Добавлены |
| 33 | user-flows.md §5/§10 | «Квизы → предмет» как вход | Кнопка называется «📖 Подготовка» |
| 34 | user-flows, features, content-authoring, study_materials | Ситуационные квизы и MCQ описаны как рабочие режимы | Их официальный контент есть только у скрытого ОПМ → **оба режима сейчас недостижимы из меню**; живы только задачи (математика) и флэш-карты (бухучёт) |
| 35 | docs/features §11 | «двухшаговое подтверждение» удаления | Команда → одно inline-подтверждение; плюс не описанные ранее ранние проверки (главный админ и «данных нет») |
...
Пункты 21–27 — третий проход по документам, которые я на первых двух
только проштамповал, но не прочитал целиком: политика конфиденциальности
и тело `admin_commands.md`. Правки в PRIVACY существенны: это юридический
документ, а перечень собираемых данных в нём был неполным.

Пункты 28–35 — четвёртый проход, уже механический: скрипты сверили каждый
идентификатор, путь и число из документации с кодом и файлами (56 проверок,
таблица тестов по областям, таксономия событий, список алиасов экспорта,
ключи локалей). Самая существенная находка — №34: два из четырёх учебных
режимов сейчас недостижимы, потому что их контент лежит у скрытого предмета.
