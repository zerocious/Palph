...
> **Doc sync:** 2026-09-13 · сверено с `bot.py`:
> **21** админская команда, из них **4** только для главного админа.
> Задокументированы все, лишних нет.
...
Справка по админским командам прямо в чате — то же, что этот файл, но
короче и всегда синхронно с кодом (текст собирается в `cmd_help`).

Обычный пользователь получает вместо неё подсказку открыть **❓ FAQ** или
написать вопрос в чат, поэтому `/help` не входит в дефолтный `/`-пикер.

**Что показывает:**
- «Общение с пользователями» — `/reply`, `/broadcast`
- «Диагностика» — `/notif_status`, `/help`
- «📊 PA-аналитика» — все 14 аналитических команд одним списком
- «👑 Только главный админ» — блок появляется, **только если**
  `message.from_user.id == MAIN_ADMIN_ID`: `/addadmin`, `/rmadmin`,
  `/listadmins`, `/backup`
- «Общие команды» — `/start`, `/stop`, `/cancel`, `/skip` и напоминание
  о кнопках главного меню
