...
# сами, а не из переданных данных.
#
# Две ошибки, которые здесь уже были:
#
# 1. Захардкоженная дата (2026-W21) делала тесты бомбой замедленного
#    действия — они проходили только в течение самой той недели, а потом
#    навсегда краснели: гранты писались в W21, рендер читал текущую
#    неделю и видел пустой лидерборд.
#
# 2. datetime.now() — это время КОНТЕЙНЕРА, а рендер резолвит неделю в
#    TZ пользователя (users.timezone, по умолчанию Europe/Moscow). В UTC
#    они расходятся каждое воскресенье с 21:00 до полуночи: контейнер
#    ещё в одной неделе, пользователь уже в следующей.
#
# Поэтому берём user_now() (см. conftest) — «сейчас» в том же поясе, в
# котором его увидит рендер — и выводим ключи тем же helper'ом, что и
# продакшн. replace() меняет только время суток, не дату, поэтому
# `%G-W%V` якоря по построению совпадает с тем, что посчитает рендер.
# 14:30 — просто детерминированный полдень, на выбор недели не влияет.
NOW = user_now().replace(hour=14, minute=30, second=0, microsecond=0)
