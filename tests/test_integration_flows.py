...
# продакшн (services.user_calendar_keys). replace() меняет только время
# суток, не дату — значит `%G-W%V` якоря по построению совпадает с тем,
# что посчитает рендер из datetime.now(), в любой день недели и в любую
# неделю. 14:30 — просто детерминированный полдень (как в исходном
# якоре), на выбор недели он не влияет.
#
# Остаточный риск: прогон, пересекающий полночь воскресенья, разъедет
# якорь и рендер по неделям. ~30 секунд на неделю — принимаем.
